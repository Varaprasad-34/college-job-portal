...
router.post('/register', [
  ...
  body('graduationYear').optional().isInt({ min: 1970, max: new Date().getFullYear() })
], async (req, res) => {
  try {
    ...
    const { name, email, password, role, major, graduationYear, bio } = req.body;
...
router.put('/profile', [
  ...
  body('linkedinProfile').optional().matches(/^https:\/\/www\.linkedin\.com\/in\/[a-zA-Z0-9-]+\/?$/)
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
